...
export default function otherFeatures() {
    elemenetGetId('loadingAn').onclick = function () {
        if (elemenetGetId('loadingAn').className == 'otherFeaturesBefore') {
            ...
            browser.storage.local.get(['dlMode'], function (budget) {
            ...
            browser.storage.local.set({ 'loadingAn': 'show' });
        } else {
            ...
            browser.storage.local.set({ 'loadingAn': 'hide' });
        }
    }
    elemenetGetId('setLoadingTime').onclick = function () {
        ...
        browser.storage.local.set({ 'loadingTime': loadingTime });
    }
}