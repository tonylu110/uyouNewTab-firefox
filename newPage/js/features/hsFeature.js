...
export function hsFeature() {
    ...
    hideHS.onclick = function () {
        if (hideHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'hideHS': 'hide' });
            ...
            browser.storage.local.set({ 'hideHS': 'show' });
    ...
    reloadHS.onclick = function () {
        if (reloadHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'reloadHS': 'hide' });
            ...
            browser.storage.local.set({ 'reloadHS': 'show' });
    ...
    calHS.onclick = function () {
        if (calHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'calHS': 'hide' });
            calHS.className = 'hsFeatureHides';
            calHS.innerText = '隐藏';
            elemenetGetId('cal').hidden = true;
        } else {
            browser.storage.local.set({ 'calHS': 'show' });
    ...
    timeHS.onclick = function () {
        if (timeHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'timeHS': 'hide' });
            timeHS.className = 'hsFeatureHides';
            timeHS.innerText = '隐藏';
            elemenetGetId('clock').hidden = true;
        } else {
            browser.storage.local.set({ 'timeHS': 'show' });
    ...
    downHS.onclick = function () {
        if (downHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'downHS': 'hide' });
            downHS.className = 'hsFeatureHides';
            downHS.innerText = '隐藏';
            elemenetGetId('down').hidden = true;
        } else {
            browser.storage.local.set({ 'downHS': 'show' });
    ...
    infoHS.onclick = function () {
        if (infoHS.className == 'hsFeatures') {
            browser.storage.local.set({ 'infoHS': 'hide' });
            infoHS.className = 'hsFeatureHides';
            infoHS.innerText = '隐藏';
            elemenetGetId('info').hidden = true;
        } else {
            browser.storage.local.set({ 'infoHS': 'show' });
...
export function hsFeatureUse() {
    browser.storage.local.get(['hideHS', 'historyHS', 'reloadHS', 'calHS', 'timeHS', 'weatherHS', 'downHS', 'infoHS', 'oneHS', 'hs'], function (budget) {
